...
from docbuilder import (
    ...
    HFExtractor,
    HFIterator,
    OriginalHFDownloader,
)

#from nemo_curator.download.doc_builder import download_and_extract
from doc_builder import download_and_extract


# function to download data from huggingface sources 
def download_huggingface_sources(
    source_links_file: Optional[str] = None,
    output_dir: Optional[str] = None,
    limit: Optional[int] = None,
) -> str:
    """
    Downloads huggingface files given provided link
    Args:
        source_links_file: Path to the file containing the source links. If not provided,
            a default file path (./sources/huggingface_urls.jsonl) will be used.
        output_dir: Directory where the downloaded sources will be saved. If not provided,
            a default directory path (./data/raw/huggingface) will be used.
        limit: Maximum number of datasets to download. If provided, only the first `limit`
    ...
    if source_links_file is None:
        source_links_file = os.path.join(
            os.path.dirname(__file__), "sources", "huggingface_urls.jsonl"
        )
    ...
    if output_dir is None:
        output_dir = os.path.join(os.path.dirname(__file__), "data", "raw", "huggingface")

    print("outputdir is: ", output_dir)
    os.makedirs(output_dir, exist_ok=True)
    # Read the source links from the provided file
    dataset_names = pd.read_json(path_or_buf=source_links_file, lines=True)
    # dataset names: multiple dataset names in json format
    dataset_name = dataset_names[0].tolist()
    ...
    downloader = HFDownloader(output_dir, dataset_name[0])
    iterator = HFIterator(output_dir)
    extractor = HFExtractor()

    dataset = download_and_extract(
    dataset_name=dataset_name,
    output_paths=[os.path.join(output_dir, os.path.basename(name)) for name in dataset_name],
