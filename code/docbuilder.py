...
class OriginalHFDownloader(DocumentDownloader):
    """
    A class for downloading data from huggingface urls.
    """
    def __init__(self, download_dir: str):
        """
        Initializes the DocBuilder object.
        Args:
            download_dir the root directory for huggingface URLs
        """
        super().__init__()
    ...
    def download(self, url: str) -> str:
        ...
        with gzip.open(output_file, "wt") as file:
            file.write(dataset['code'])
        return output_file


class HFDownloader(DocumentDownloader):
    """
    A class for downloading data from huggingface dataset, define steps for downloading single dataset
    """
    def __init__(self, download_dir: str, dataset: str):
        """
        Initializes the DocBuilder object.
        Args:
            download_dir the root directory for huggingface URLs
            single dataset to be downloaded
        """
        super().__init__()
        if not os.path.isdir(download_dir):
...
class HFIterator(DocumentIterator):
    ...
    """
    # token that separates pararaphs.
    ...
    def __init__(self, download_dir):
        ...
        if not os.path.isdir(download_dir):
            os.makedirs(download_dir, exist_ok = True)
        self._download_dir = download_dir
        # all the keys in dataset per line
        self.keys = ("code", "block_summary", "detailed_global_summary", "high_level_global_summary")

    def split_contents(self, line, file_name) -> tuple:
        line = line[0]
        for key in self.keys:
            assert key in line, key + " is not in line, abort!"
        ...
        assert len(all_meta) == 4 and len(all_content) == 4, "length of all meta or all content does not equal 3!"
...
##obselete code
"""
with open("block_summary.jsonl", "w") as f, open("detailed_global_summary.jsonl", "w") as g, open("high_level_global_summary.jsonl", "w") as p:
    for idx in range(len(dataset["code"])):
        f.write(
            json.dumps({"input": dataset["code"][idx], "output": dataset["description"][idx]["block_summary"]})    
            + "\n")
        g.write(
            json.dumps({"input": dataset["code"][idx], "output": dataset["description"][idx]["detailed_global_summary"]})    
            + "\n")
        p.write(
            json.dumps({"input": dataset["code"][idx], "output": dataset["description"][idx]["high_level_global_summary"]})    
            + "\n")
f.close()
g.close()
p.close()
